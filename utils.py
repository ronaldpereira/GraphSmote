...
def get_parser():
    ...
    parser.add_argument('--epochs', type=int, default=2010, help='Number of epochs to train.')
    ...
    parser.add_argument(
        ...
        default='no',
        choices=[
            'no', 'upsampling', 'smote', 'reweight', 'embed_up', 'recon', 'newG_cls', 'recon_newG'
        ]
    )
...
def split_genuine(labels):
    ...
    for i in range(num_classes):
        ...
        test_idx = test_idx + c_idx[c_num_mat[i, 0] + c_num_mat[i, 1]:c_num_mat[i, 0] +
                                    c_num_mat[i, 1] + c_num_mat[i, 2]]
...
def print_class_acc(output, labels, class_num_list, pre='valid'):
    ...
    if labels.max() > 1:
        auc_score = roc_auc_score(
            labels.detach(), F.softmax(output, dim=-1).detach(), average='macro', multi_class='ovr'
        )
    else:
        auc_score = roc_auc_score(
    ...
    print(
        str(pre) +
        ' current auc-roc score: {:f}, current macro_F score: {:f}'.format(auc_score, macro_F)
    )
    ...
    for i in range(im_class_num):
        ...
        for j in range(c_portion):
            ...
            if adj is not None:
                if adj_new is None:
                    adj_new = adj.new(
                        torch.clamp_(adj[chosen, :] + adj[idx_neighbor, :], min=0.0, max=1.0)
                    )
                else:
                    temp = adj.new(
                        torch.clamp_(adj[chosen, :] + adj[idx_neighbor, :], min=0.0, max=1.0)
