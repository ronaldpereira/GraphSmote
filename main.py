...
if args.setting == 'upsampling':
    adj, features, labels, idx_train = utils.src_upsample(
        adj, features, labels, idx_train, portion=args.up_scale, im_class_num=im_class_num
    )
...
if args.setting != 'embed_up':
    if args.model == 'sage':
        ...
        classifier = models.Sage_Classifier(
            nembed=args.nhid, nhid=args.nhid, nclass=labels.max().item() + 1, dropout=args.dropout
        )
    elif args.model == 'gcn':
...
def train(epoch):
    ...
    if args.setting == 'recon_newG' or args.setting == 'recon' or args.setting == 'newG_cls':
        ...
        generated_edge_prob = adj_new[ori_num:, :ori_num].mean()  #edge prob for generated nodes
        print(
            "after filtering, edge acc: {:.4f}, exist_edge_prob: {:.4f}, generated_edge_prob: {:.4f}"
            .format(edge_ac.item(), exist_edge_prob.item(), generated_edge_prob.item())
        )
    ...
    print(
        'Epoch: {:05d}'.format(epoch + 1), 'loss_train: {:.4f}'.format(loss_train.item()),
        'loss_rec: {:.4f}'.format(loss_rec.item()), 'acc_train: {:.4f}'.format(acc_train.item()),
...
def test(epoch=0):
    ...
    print(
        "Test set results:", "loss= {:.4f}".format(loss_test.item()),
...
def save_model(epoch):
    ...
    torch.save(
        saved_content, 'checkpoint/{}/{}_{}_{}_{}.pth'.format(
...
def load_model(filename):
    loaded_content = torch.load(
        'checkpoint/{}/{}.pth'.format(args.dataset, filename),
